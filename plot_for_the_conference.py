...
files = sorted(glob.glob('./results/threshEqualsDiamOver3_data/random/256nodes_diameter5_cutoffinf*.xlsx'))
...
for idx, f in enumerate(files):
    ...
    ax3.plot(
        ...
        mean_stretch.values,
        marker='o',
        label=f'Stretch$_{'P'}$$_{'A'}$$_{'r'}$$_{'r'}$$_{'o'}$$_{'w'} $(${'err'}$ ≤ {actual_cutoff})',
        color=cmap(2 * idx + 1),
        linewidth=2
    )
...
ax3.set_ylabel('Stretch', fontsize=12)
...
ax3.set_title(f'Random network of {node_count} nodes', fontsize=12)
...
plt.tight_layout()
plt.ylim(0, 5)
folder = "results/threshEqualsDiamOver3_plots/for_conference"
